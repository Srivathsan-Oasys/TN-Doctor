...
class DetailedRegistrationViewModel(
    ...
    fun getAllDepartmentAll(
        req: GetAllDepartmentRequestModel?,
        favAddAllDepartmentCallBack: RetrofitCallback<FavAddAllDepatResponseModel>
    ) {
        if (!Utils.isNetworkConnected(getApplication())) {
            errorText.value = getApplication<Application>().getString(R.string.no_internet)
        ...
        apiService?.getSearchDepartmentAll(
            AppConstants.BEARER_AUTH + userDataStoreBean?.access_token,
            userDataStoreBean?.uuid!!, facility_id!!,
            req
        )?.enqueue(RetrofitMainCallback(favAddAllDepartmentCallBack))
    ...
    fun getVisits(
        facility_id: Int?,
        query: GetPatientAllVisitsRequest,
        complaintSearchRetrofitCallBack: RetrofitCallback<GetPatientAllVisitsResponse>
    ) {
        if (!Utils.isNetworkConnected(getApplication())) {
            errorText.value = getApplication<Application>().getString(R.string.no_internet)
            return
        ...
        val apiService = aiiceApplication.getRetrofitService()
        val userDataStoreBean = userDetailsRoomRepository.getUserDetails()
        apiService?.getPatientAllVisits(
            "en",
            AppConstants.BEARER_AUTH + userDataStoreBean?.access_token,
            userDataStoreBean?.uuid!!,
            facility_id!!,
    ...
    fun setReferrals(
        facility_id: Int?,
        query: String,
        complaintSearchRetrofitCallBack: RetrofitCallback<GetPatientAllReferralsResponse>
    ) {
        if (!Utils.isNetworkConnected(getApplication())) {
            errorText.value = getApplication<Application>().getString(R.string.no_internet)
            return
        ...
        val apiService = aiiceApplication.getRetrofitService()
        val userDataStoreBean = userDetailsRoomRepository.getUserDetails()
        apiService?.getPatientAllReferrals(
            "en",
            AppConstants.BEARER_AUTH + userDataStoreBean?.access_token,
            userDataStoreBean?.uuid!!,
            facility_id!!,
