...
import com.hmis_tn.doctor.ui.emr_workflow.investigation_result.model.InvestigationResultResponseContent
import com.hmis_tn.doctor.ui.emr_workflow.radiology_result.ui.RadiologyResultChildAdapter
...
class InvestigationResultParentAdapter(
    ...
    private var radiologyResultList: ArrayList<InvestigationResultResponseContent>
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        /*       try {
                   orderNumString = radiologyResultList[position].toString()
                   val resultListList = radiologyResultList[position]
                   ...
                   if (resultListList.repsonse.isNotEmpty() && !resultListList.repsonse[position].result_value.isNullOrEmpty())
                       holder.DescriptionText2.text =
                           resultListList.repsonse.get(position).result_value ?: ""
                   holder.recyclerView.layoutManager = LinearLayoutManager(mContext)
                   val myOrderChildAdapter = InvestigationResultChildAdapter(mContext)
                   ...
                   if (!resultListList.repsonse.isNullOrEmpty())
                       myOrderChildAdapter.setData(resultListList?.repsonse)
        ...
        try {
            ...
            val date = utils.displayInvesDate(
                resultListList.order_request_date,
                "yyyy-MM-dd HH:mm:ss"
            )

            holder.dateTextView.text =
                date + "-" + resultListList.test_master + "-" + "-" + resultListList.department + "-" + resultListList.encounter_type
            holder.DescriptionText.text = resultListList.test_master
    ...
    fun setData(labResultLIst: ArrayList<InvestigationResultResponseContent>) {
