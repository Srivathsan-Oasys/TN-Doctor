...
class RadiologyViewModel(
    ...
    fun getRadiologyFavourites(
        emrWorkFlowRetrofitCallBack: RetrofitCallback<RadiologyFavResponse>,
        departmentID: Int?
    ) {
        ...
        apiService?.getRadiologyFavourites(
    ...
    fun getRadiologyType(
        labTypeRetrofitCallback: RetrofitCallback<RadiologyTypeResponseModel>,
        facilityID: Int?
    ) {
    ...
    fun getToLocation(
        ...
        val apiService = aiiceApplication.getRetrofitService()
        val userDataStoreBean = userDetailsRoomRepository?.getUserDetails()
        apiService?.getToLocationRadilogyTest(
    ...
    fun rmisUpdate(
        facility_id: Int?,
        radiologyUpdateRequest: RadiologyRequestModel,
        configFinalRetrofitCallBack: RetrofitCallback<RadiologyUpdateResponse>
    ) {
        if (!Utils.isNetworkConnected(getApplication())) {
            errorText.value = getApplication<Application>().getString(R.string.no_internet)
            return
        ...
        val aiiceApplication = HmisApplication.get(getApplication())
        val apiService = aiiceApplication.getRetrofitService()
        apiService?.rmisUpdate(
            AppConstants.BEARER_AUTH + userDataStoreBean?.access_token,
            userDataStoreBean?.uuid!!, facility_id!!,
            radiologyUpdateRequest
        )?.enqueue(RetrofitMainCallback(configFinalRetrofitCallBack))
        return
