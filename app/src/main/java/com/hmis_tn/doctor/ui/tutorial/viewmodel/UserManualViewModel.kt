...
class UserManualViewModel(
    ...
    fun getRoleControlActivity(
        jsonBody: JSONObject,
        GetLabTestApprovalListRetrofitCallback: RetrofitCallback<RoleControlActivityResponseModel>
    ) {

        Log.e("getRoleControlActivity", "inside")
        val userDataStoreBean = userDetailsRoomRepository?.getUserDetails()
        ...
        apiService?.getRoleControlActivity(
            AppConstants.ACCEPT_LANGUAGE_EN,
            AppConstants.BEARER_AUTH + userDataStoreBean?.access_token,
            userDataStoreBean?.uuid!!, facility_id!!, userDataStoreBean.user_name,
            body
