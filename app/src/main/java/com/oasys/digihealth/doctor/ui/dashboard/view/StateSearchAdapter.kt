...
import com.oasys.digihealth.doctor.ui.dashboard.model.registration.State

class StateSearchAdapter(
    context: Context, @LayoutRes
    private val layoutResource: Int,
    private var allPois: ArrayList<State>
) :
    ArrayAdapter<State>(context, layoutResource, allPois),
    Filterable {
    private var mPois: List<State> = allPois

    override fun getCount(): Int {
        return mPois.size
    }

    override fun getItem(p0: Int): State? {
        return mPois.get(p0)
    }
    ...
    override fun getFilter(): Filter {
        return object : Filter() {
            override fun publishResults(
                ...
                mPois = filterResults.values as List<State>
    ...
    fun setData(allData: ArrayList<State>) {
